...
- 🎯 Entry points to execute your program using `python -m <fastapi_sample>` or `$ fastapi_sample` with basic CLI argument parsing.
...
[![codecov](https://codecov.io/gh/khanhpnp90/fastapi-sample/branch/main/graph/badge.svg?token=fastapi-sample_token_here)](https://codecov.io/gh/khanhpnp90/fastapi-sample)
[![CI](https://github.com/khanhpnp90/fastapi-sample/actions/workflows/main.yml/badge.svg)](https://github.com/khanhpnp90/fastapi-sample/actions/workflows/main.yml)

Awesome fastapi_sample created by khanhpnp90

## Install

from source
```bash
git clone https://github.com/khanhpnp90/fastapi-sample fastapi_sample
cd fastapi_sample
make install
```
...
```bash
pip install fastapi_sample
```

## Executing

```bash
$ fastapi_sample run --port 8080
```

or

```bash
python -m fastapi_sample run --port 8080
```

or

```bash
$ uvicorn fastapi_sample:app
```
...
```bash
❯ fastapi_sample --help
Usage: fastapi_sample [OPTIONS] COMMAND [ARGS]...
...
```bash
❯ fastapi_sample create-user --help
Usage: fastapi_sample create-user [OPTIONS] USERNAME PASSWORD
...
```bash
fastapi_sample create-user admin admin --superuser
```
...
Run with `fastapi_sample run` and access http://127.0.0.1:8000/docs

![](https://raw.githubusercontent.com/rochacbruno/fastapi-project-template/master/docs/api.png)


**For some api calls you must authenticate** using the user created with `fastapi_sample create-user`.
...
-----------------------------------------------------
fastapi_sample/__init__.py              4      0   100%
fastapi_sample/app.py                  16      1    94%
fastapi_sample/cli.py                  21      0   100%
fastapi_sample/config.py                5      0   100%
fastapi_sample/db.py                   10      0   100%
fastapi_sample/models/__init__.py       0      0   100%
fastapi_sample/models/content.py       47      1    98%
fastapi_sample/routes/__init__.py      11      0   100%
fastapi_sample/routes/content.py       52     25    52%
fastapi_sample/routes/security.py      15      1    93%
fastapi_sample/routes/user.py          52     26    50%
fastapi_sample/security.py            103     12    88%
-----------------------------------------------------
TOTAL                               336     66    80%
...
```py
from fastapi_sample.config import settings
```
...
> `dynaconf_merge` is a boolean that tells if the settings should be merged with the default settings defined in fastapi_sample/default.toml.

### As environment variables
```bash
export fastapi_sample_KEY=value
export fastapi_sample_KEY="@int 42"
export fastapi_sample_KEY="@jinja {{ this.db.uri }}"
export fastapi_sample_DB__uri="@jinja {{ this.db.uri | replace('db', 'data') }}"
```
...
```bash
fastapi_sample_ENV=production fastapi_sample run
```
